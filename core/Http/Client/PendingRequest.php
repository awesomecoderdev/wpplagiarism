...
use AwesomeCoder\Http\Client\Events\ConnectionFailed;
use AwesomeCoder\Http\Client\Events\RequestSending;
use AwesomeCoder\Http\Client\Events\ResponseReceived;
...
class PendingRequest
{
     ...
     *
     * @var \AwesomeCoder\Http\Client\Factory|null
     */
    protected $factory;
     ...
     *
     * @var \AwesomeCoder\Support\Collection|null
     */
     ...
     *
     * @var \AwesomeCoder\Http\Client\Request|null
     */
    protected $request;
     ...
     *
     * @param  \AwesomeCoder\Http\Client\Factory|null  $factory
     * @return void
     */
     ...
     * @return \AwesomeCoder\Http\Client\Response
     */
    ...
    protected function sendRequest(string $method, string $url, array $options = [])
    {
        ...
        $pluginData = $this->parseRequestData($method, $url, $options);
    ...
    protected function parseRequestData($method, $url, array $options)
    {
        ...
        $pluginData = $options[$this->bodyFormat] ?? $options['query'] ?? [];

        $urlString = Str::of($url);

        if (empty($pluginData) && $method === 'GET' && $urlString->contains('?')) {
            $pluginData = (string) $urlString->after('?');
        }

        if (is_string($pluginData)) {
            parse_str($pluginData, $parsedData);

            $pluginData = is_array($parsedData) ? $parsedData : [];
        }

        if ($pluginData instanceof JsonSerializable) {
            $pluginData = $pluginData->jsonSerialize();
        }

        return is_array($pluginData) ? $pluginData : [];
    }
            ...
            return function ($request, $options) use ($handler) {
                ...
                return $promise->then(function ($response) use ($request, $options) {
                    $this->factory?->recordRequestResponsePair(
                        (new Request($request))->withData($options['PLUGIN_data']),
        ...
        return function ($handler) {
            return function ($request, $options) use ($handler) {
                $response = ($this->stubCallbacks ?? collect())
                    ->map
                    ->__invoke((new Request($request))->withData($options['PLUGIN_data']), $options)
                    ->filter()
                    ->first();
