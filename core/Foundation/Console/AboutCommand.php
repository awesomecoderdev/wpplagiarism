...
class AboutCommand extends Command
{
            ...
            ->map(
                fn ($items) => collect($items)
                    ->map(function ($value) {
                        ...
                        if (is_string($value)) {
                            $value = $this->plugin->make($value);
                        }

                        return collect($this->plugin->call($value))
    ...
    protected function gatherApplicationInformation()
    {
        static::addToSection('Environment', fn () => [
            ...
            'Maintenance Mode' => $this->plugin->isDownForMaintenance() ? '<fg=yellow;options=bold>ENABLED</>' : 'OFF',
        ]);

        static::addToSection('Cache', fn () => [
            'Config' => $this->plugin->configurationIsCached() ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
            'Events' => $this->plugin->eventsAreCached() ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
            'Routes' => $this->plugin->routesAreCached() ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
            'Views' => $this->hasPhpFiles($this->plugin->storagePath('framework/views')) ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
        ]);
