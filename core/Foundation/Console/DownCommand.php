...
class DownCommand extends Command
{
    ...
    {
        try {
            if ($this->plugin->maintenanceMode()->active()) {
                $this->components->info('Application is already down.');

                return 0;
            }

            $this->plugin->maintenanceMode()->activate($this->getDownFilePayload());
            ...
            $this->plugin->get('events')->dispatch(new MaintenanceModeEnabled());
    ...
    {
        try {
            return $this->plugin->make(PreventRequestsDuringMaintenance::class)->getExcludedPaths();
        } catch (Throwable $e) {
            return [];
        }
    }
