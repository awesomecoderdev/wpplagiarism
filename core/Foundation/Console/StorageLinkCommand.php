...
class StorageLinkCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($this->links() as $link => $target) {
            ...
            if (is_link($link)) {
                $this->plugin->make('files')->delete($link);
            }

            if ($relative) {
                $this->plugin->make('files')->relativeLink($target, $link);
            } else {
                $this->plugin->make('files')->link($target, $link);
            }
    ...
    {
        return $this->plugin['config']['filesystems.links'] ??
