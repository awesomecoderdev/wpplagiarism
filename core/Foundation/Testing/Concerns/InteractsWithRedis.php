...
use AwesomeCoder\Redis\RedisManager;
...
trait InteractsWithRedis
{
     ...
     *
     * @var \AwesomeCoder\Redis\RedisManager[]
     */
    private $redis;
    ...
    public function setUpRedis()
    {
        ...
        $plugin = $this->plugin ?? new Application;
        $host = Env::get('REDIS_HOST', '127.0.0.1');
        $port = Env::get('REDIS_PORT', 6379);

        foreach ($this->redisDriverProvider() as $driver) {
            $this->redis[$driver[0]] = new RedisManager($plugin, $driver[0], [
                'cluster' => false,
                'options' => [
                    'prefix' => 'test_',
                ],
