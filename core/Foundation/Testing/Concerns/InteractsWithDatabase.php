...
use AwesomeCoder\Support\Facades\DB;
use AwesomeCoder\Testing\Constraints\CountInDatabase;
use AwesomeCoder\Testing\Constraints\HasInDatabase;
use AwesomeCoder\Testing\Constraints\NotSoftDeletedInDatabase;
use AwesomeCoder\Testing\Constraints\SoftDeletedInDatabase;
...
trait InteractsWithDatabase
{
     ...
     *
     * @param  \AwesomeCoder\Database\Eloquent\Model|string  $table
     * @param  array  $data
    ...
    protected function assertSoftDeleted($table, array $data = [], $connection = null, $deletedAtColumn = 'deleted_at')
    {
        ...
        $this->assertThat(
            $this->getTable($table),
            new SoftDeletedInDatabase($this->getConnection($connection, $table), $data, $deletedAtColumn)
        );
    ...
    protected function assertNotSoftDeleted($table, array $data = [], $connection = null, $deletedAtColumn = 'deleted_at')
    {
        ...
        $this->assertThat(
            $this->getTable($table),
            new NotSoftDeletedInDatabase($this->getConnection($connection, $table), $data, $deletedAtColumn)
        );
