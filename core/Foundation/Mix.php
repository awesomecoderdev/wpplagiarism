...
class Mix
{
    ...
    public function __invoke($path, $manifestDirectory = '')
    {
        ...
        if (is_file(public_path($manifestDirectory . '/hot'))) {
            ...
            $customUrl = plugin('config')->get('app.mix_hot_proxy_url');
        ...
        if (!isset($manifest[$path])) {
            ...
            if (!plugin('config')->get('app.debug')) {
                report($exception);
        ...
        return new HtmlString(plugin('config')->get('app.mix_url') . $manifestDirectory . $manifest[$path]);
    }
}
