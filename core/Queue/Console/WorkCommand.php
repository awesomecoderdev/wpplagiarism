...
use AwesomeCoder\Contracts\Queue\Job;
use AwesomeCoder\Queue\Events\JobFailed;
use AwesomeCoder\Queue\Events\JobProcessed;
use AwesomeCoder\Queue\Events\JobProcessing;
use AwesomeCoder\Queue\Events\JobReleasedAfterException;
use AwesomeCoder\Queue\Worker;
use AwesomeCoder\Queue\WorkerOptions;
...
class WorkCommand extends Command
{
     ...
     *
     * @param  \AwesomeCoder\Queue\Worker  $worker
     ...
     *
     * @return \AwesomeCoder\Queue\WorkerOptions
     */
    ...
    {
        $this->plugin['events']->listen(JobProcessing::class, function ($event) {
            $this->writeOutput($event->job, 'starting');
        });

        $this->plugin['events']->listen(JobProcessed::class, function ($event) {
            $this->writeOutput($event->job, 'success');
        });

        $this->plugin['events']->listen(JobReleasedAfterException::class, function ($event) {
            $this->writeOutput($event->job, 'released_after_exception');
        });

        $this->plugin['events']->listen(JobFailed::class, function ($event) {
    ...
    {
        $queueTimezone = $this->plugin['config']->get('queue.output_timezone');

        if (
            $queueTimezone &&
            $queueTimezone !== $this->plugin['config']->get('app.timezone')
        ) {
     ...
     *
     * @param  \AwesomeCoder\Queue\Events\JobFailed  $event
     * @return void
     */
    protected function logFailedJob(JobFailed $event)
    {
        $this->plugin['queue.failer']->log(
    ...
    {
        return $this->option('force') ? false : $this->plugin->isDownForMaintenance();
    }
}
