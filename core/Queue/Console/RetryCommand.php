...
use AwesomeCoder\Contracts\Encryption\Encrypter;
use AwesomeCoder\Queue\Events\JobRetryRequested;
...
class RetryCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($ids as $id) {
            $job = $this->plugin['queue.failer']->find($id);

            if (is_null($job)) {
                $this->components->error("Unable to find failed job with ID [{$id}].");
            } else {
                $this->plugin['events']->dispatch(new JobRetryRequested($job));

                $this->components->task($id, fn () => $this->retryJob($job));

                $this->plugin['queue.failer']->forget($id);
            }
        }
    ...
    protected function getJobIds()
    {
        ...
        if (count($ids) === 1 && $ids[0] === 'all') {
            return Arr::pluck($this->plugin['queue.failer']->all(), 'id');
        }
    ...
    {
        $ids = collect($this->plugin['queue.failer']->all())
            ->where('queue', $queue)
            ->pluck('id')
            ->toArray();
    ...
    {
        $this->plugin['queue']->connection($job->connection)->pushRaw(
    ...
    protected function refreshRetryUntil($payload)
    {
        ...
        } elseif ($this->plugin->bound(Encrypter::class)) {
            $instance = unserialize($this->plugin->make(Encrypter::class)->decrypt($payload['data']['command']));
        }
