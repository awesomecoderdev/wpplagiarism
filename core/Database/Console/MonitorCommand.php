...
namespace AwesomeCoder\Database\Console;

use AwesomeCoder\Contracts\Events\Dispatcher;
use AwesomeCoder\Database\ConnectionResolverInterface;
use AwesomeCoder\Database\Events\DatabaseBusy;
...
class MonitorCommand extends DatabaseInspectionCommand
{
     ...
     *
     * @param  \AwesomeCoder\Database\ConnectionResolverInterface  $connection
    ...
    protected function parseDatabases($databases)
    {
        return collect(explode(',', $databases))->map(function ($database) {
            if (!$database) {
                $database = $this->plugin['config']['database.default'];
            }
     ...
     *
     * @param  \AwesomeCoder\Support\Collection  $databases
     * @return void
     */
