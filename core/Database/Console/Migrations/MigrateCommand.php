...
namespace AwesomeCoder\Database\Console\Migrations;

use AwesomeCoder\Console\ConfirmableTrait;
use AwesomeCoder\Contracts\Console\Isolatable;
use AwesomeCoder\Contracts\Events\Dispatcher;
use AwesomeCoder\Database\Events\SchemaLoaded;
use AwesomeCoder\Database\Migrations\Migrator;
use AwesomeCoder\Database\SQLiteDatabaseDoesNotExistException;
...
class MigrateCommand extends BaseCommand implements Isolatable
{
     ...
     *
     * @var \AwesomeCoder\Database\Migrations\Migrator
     */
    protected $migrator;
     ...
     *
     * @param  \AwesomeCoder\Database\Migrations\Migrator  $migrator
    ...
    {
        if ($this->plugin['config']->get("database.connections.{$connection->getName()}.database") !== $connection->getDatabaseName()) {
            return false;
        }
        ...
        try {
            $this->plugin['config']->set("database.connections.{$connection->getName()}.database", null);

            $this->plugin['db']->purge();
            ...
            $this->plugin['config']->set("database.connections.{$connection->getName()}.database", $connection->getDatabaseName());
        }
    }
